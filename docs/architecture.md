...
The software stack relies on Python 3.9+, standard libraries, Flask, HTML/Vanilla JS/CSS, and `ntfy.sh` for push notifications. 
...
- **State Management:** Operates statelessly inside memory but frequently persists its findings to local JSON files (`config.json`, `state.json`, `group_rules.json`, `notification_log.json`).
...
- **Execution:** `app.py` serves a Flask REST API and hosts the frontend static files. Productively served via `waitress`.
...
1. **`config.json`**: Holds thresholds, ntfy credentials, and ignored groups. Editable via UI, consumed by daemon.
...
- **Zero-Secret Logging:** Strict sentinel values (`_TOKEN_SENTINEL`) ensure passwords and API keys are never rendered back to the browser or disk logs.
- **Dependency Minimization:** Heavily relies on the Python standard library (e.g. `ipaddress`, `socket`, `json`, `urllib`). Only `requests` and `flask` are notable third-party dependencies.
