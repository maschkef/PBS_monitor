...
> nano .env  # Editieren und API_KEY eintragen
> docker compose up -d
> 
> # Dashboard öffnen unter http://localhost:5111
> ```
> 
...
**Für die Docker-Bereitstellung (empfohlen):**
...
# Optional, für stabile Sessions über Neustarts hinweg:
# Einen beliebigen langen Zufallstext eintragen. Wenn leer, wird beim Start automatisch ein Schlüssel generiert (Sessions gehen bei Neustart verloren).
WEBUI_SECRET_KEY=
```
...
- **Konfigurierbare Benachrichtigungs-Prioritäten** — ntfy-Priorität pro Severity-Tier (Warning / Critical) festlegen und optional für jeden einzelnen Alert-Typ überschreiben (z. B. "GC Never Ran" unabhängig von anderen Warnings leiser stellen, oder einen Typ auf `ignore` setzen um ihn komplett zu unterdrücken); dieselbe Skala 1–5 nutzt auch der Quiet-Hours-Mindestschwellwert
...
Type=simple
User=DEIN_USER
WorkingDirectory=/pfad/zu/PBS_monitor
ExecStart=/pfad/zu/PBS_monitor/.venv/bin/python -m alerting.monitor --daemon
Restart=on-failure
RestartSec=10
